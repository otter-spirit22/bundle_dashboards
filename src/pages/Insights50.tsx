...
type Badge = "urgent" | "opportunity" | "good" | "warn" | string;
type Insight = { n: number; title: string; count: string | number; badge: Badge; def: string };
...
// Map each insight to a category (tweak as you like)
const INSIGHT_CATEGORY_MAP: Record<number, InsightCategory> = {
  ...
  43: "Retention Radar",
  44: "Retention Radar",
  45: "Growth Opportunities",
  46: "Growth Opportunities",
  47: "Retention Radar",
  48: "Growth Opportunities",
  49: "Risk & Compliance",
  50: "Risk & Compliance",
};
...
export default function Insights50() {
  ...
  const [managerOptions, setManagerOptions] = useState<string[]>(["Unassigned"]);
  const [selectedManager, setSelectedManager] = useState<string>("All");
  const [selectedCategory, setSelectedCategory] = useState<InsightCategory | "All">("All");
  const [selectedMetric, setSelectedMetric] = useState<number | "All">("All");

  const N = rows.length;

  // Build a filtered copy of rows by manager (if selected)
  const rowsForFilters = useMemo(() => {
    if (!rows.length) return [];
    if (!managerKey || selectedManager === "All") return rows;
    return rows.filter((r) => {
      const val = String(r[managerKey] ?? "Unassigned");
      return val === selectedManager;
    });
  }, [rows, managerKey, selectedManager]);

  // Compute insights & matched rows from filtered rows
  const { insights, matchesMap } = useMemo(() => {
    const computed = computeAllInsights(rowsForFilters, fields);
    return { insights: computed.insights, matchesMap: computed.matches };
  }, [rowsForFilters, fields]);

  // Keep matches state for modal export
  useMemo(() => setMatches(matchesMap), [matchesMap]);

  // Build "metric" list (insight titles) for the filter dropdown
  const metricOptions = useMemo(() => {
    return ["All", ...insights.map((i) => `${i.n}. ${i.title}`)];
  }, [insights]);

  // Apply Category/Metric filters to the displayed list
  const visibleInsights = useMemo(() => {
    return insights.filter((ins) => {
      const byCategory =
        selectedCategory === "All" ? true : INSIGHT_CATEGORY_MAP[ins.n] === selectedCategory;
      const byMetric =
        selectedMetric === "All" ? true : ins.n === selectedMetric;
      return byCategory && byMetric;
    });
  }, [insights, selectedCategory, selectedMetric]);
  ...
  function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    Papa.parse<Row>(file, {
      ...
      complete: (res) => {
        ...
        // Detect a service manager column and options
        const candidateKeys = ["service_manager","Service Manager","manager","csr","CSR"];
        const found = f.find((k) => candidateKeys.includes(k)) || null;
        setManagerKey(found);
        ...
        if (found) {
          const opts = Array.from(
            new Set(
              parsed
                .map((r) => String(r[found] ?? "").trim())
                .filter((s) => s.length > 0)
            )
          );
          setManagerOptions(opts.length ? ["All", ...opts] : ["All", "Unassigned"]);
          setSelectedManager("All");
        } else {
          // Demo fallback
          setManagerOptions(["All", "Unassigned", "Alex Carter", "Jamie Lee", "Taylor Morgan"]);
          setSelectedManager("All");
        }
      }
    });
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="Retention Radar">Retention Radar</option>
              <option value="Service Drain">Service Drain</option>
              <option value="Risk & Compliance">Risk & Compliance</option>
            </select>
          </div>

          {/* Metric / Specific Insight */}
          <div className="flex items-center gap-2">
            <label className="text-xs opacity-70">Metric</label>
            <select
              className="rounded bg-white/5 border border-white/10 p-1 text-sm"
              value={selectedMetric === "All" ? "All" : String(selectedMetric)}
              onChange={(e) => {
                const v = e.target.value;
                if (v === "All") setSelectedMetric("All");
                else {
                  const n = Number(v.split(".")[0]); // "12. Title"
                  setSelectedMetric(isNaN(n) ? "All" : n);
                }
              }}
            >
              {metricOptions.map((label, i) => (
                <option key={i} value={label === "All" ? "All" : label}>
                  {label}
                </option>
              ))}
            </select>
          ...
          <table className="min-w-full border border-white/10">
            ...
            <tbody>
              {visibleInsights.map((ins) => {
                ...
                const category = INSIGHT_CATEGORY_MAP[ins.n];
                return (
                  <tr key={ins.n} className="border-t border-white/10">
                    ...
                    <td className="p-2 align-top text-sm opacity-80">{category}</td>
              ...
              {visibleInsights.length === 0 && (
                <tr>
                  <td colSpan={6} className="p-4 text-sm text-slate-400">
...
/** ---------- Insight engine (unchanged logic, wrapped in a function) ---------- */
function computeAllInsights(rows: Row[], fields: string[]) {
